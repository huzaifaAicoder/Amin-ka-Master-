...
export default function AuthScreen() {
  ...
  const developerTapSequence = useRef({ count: 0, startedAt: 0 });
  ...
  const registerDeveloperTap = () => {
    const now = Date.now();
    const sequence = developerTapSequence.current;
    const withinWindow = now - sequence.startedAt <= 3_000;
    const count = withinWindow ? sequence.count + 1 : 1;
    developerTapSequence.current = { count, startedAt: withinWindow ? sequence.startedAt : now };
    if (count !== 7) return;
    developerTapSequence.current = { count: 0, startedAt: 0 };
