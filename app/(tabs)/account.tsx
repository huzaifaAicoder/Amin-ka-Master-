  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[COLORS.indigo]} tintColor={COLORS.indigo} />}>
        ...
        <View style={styles.group}><AccountRow icon="bookmark" label="Saved Shorts" detail="Your saved quick-learning videos" onPress={() => router.push("/saved-shorts" as never)} /><AccountRow icon="assignment" label="Practice tests" detail="Attempts and results" onPress={() => router.push("/tests")} /><AccountRow icon="videocam" label="Live classes" detail="Upcoming and completed sessions" onPress={() => router.push("/live")} /><AccountRow icon="terrain" label="Amin Master Toolkit" detail="GPS, official portals, and a digital compass" onPress={() => router.push("/amin-toolkit" as never)} /><AccountRow icon="family-restroom" label="Parent/Guardian reports" detail="Control consent for aggregate progress sharing" onPress={() => router.push("/guardian-reports" as never)} /><AccountRow icon="notifications-none" label="Notifications" detail="Course updates and announcements" onPress={() => router.push("/notifications")} /></View>
