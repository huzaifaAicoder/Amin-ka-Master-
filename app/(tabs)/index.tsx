...
export default function HomeScreen() {
  ...
  const t = (english: string, hindi: string) => label(english, hindi);
  const languageCode = language === "english" ? "EN" : language === "hindi" ? "हि" : "EN/हि";
    ...
    <ScreenContainer containerClassName="bg-background" className="px-5" edges={["top", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[COLORS.indigo]} tintColor={COLORS.indigo} />}>
        <View style={styles.header}>
          <View>
            <Text style={styles.eyebrow}>{user ? t("YOUR LEARNING SPACE", "आपका अध्ययन स्थान") : t("LEARN. MEASURE. MASTER.", "सीखें। मापें। महारत पाएं।")}</Text>
            <Text style={styles.greeting}>{user?.fullName ? `${t("Hello", "नमस्ते")}, ${user.fullName.split(" ")[0]}` : appName}</Text>
          </View>
          <View style={styles.headerActions}><Pressable accessibilityLabel={t(`Change interface language, currently ${language}`, `भाषा बदलें, वर्तमान ${language}`)} onPress={cycleLanguage} style={({ pressed }) => [styles.languageButton, pressed && styles.pressed]}><MaterialIcons name="translate" size={18} color={COLORS.indigo} /><Text style={styles.languageCode}>{languageCode}</Text></Pressable><Pressable accessibilityLabel={t("Notifications", "सूचनाएं")} onPress={() => user ? router.push("/notifications") : router.push("/auth")} style={({ pressed }) => [styles.notificationButton, pressed && styles.pressed]}><MaterialIcons name="notifications-none" size={24} color={COLORS.indigo} /></Pressable></View>
        </View>

        {activeLearning ? (
          <Card style={styles.continueCard}>
            <View style={styles.continueTop}><Tag label={t("CONTINUE LEARNING", "सीखना जारी रखें")} tone="saffron" /><Text style={styles.percent}>{Math.round((activeLearning.progress.completedLessons / Math.max(activeLearning.progress.lessonCount, 1)) * 100)}%</Text></View>
            <Text style={styles.continueTitle}>{activeLearning.course.title}</Text>
            <Text style={styles.continueBody}>{language === "hindi" ? `${activeLearning.progress.lessonCount} में से ${activeLearning.progress.completedLessons} पाठ पूरे` : language === "bilingual" ? `${activeLearning.progress.completedLessons} of ${activeLearning.progress.lessonCount} lessons complete · ${activeLearning.progress.lessonCount} में से ${activeLearning.progress.completedLessons} पाठ पूरे` : `${activeLearning.progress.completedLessons} of ${activeLearning.progress.lessonCount} lessons complete`}</Text>
            ...
            <PrimaryButton label={t("Resume course", "पाठ्यक्रम जारी रखें")} icon="play-arrow" onPress={() => router.push(`/course/${activeLearning.course.slug}`)} />
          </Card>
        ) : (
        ...
        {showLive && upcomingClass ? (
          <Pressable onPress={() => router.push("/live")} style={({ pressed }) => [styles.liveStrip, pressed && styles.pressed]}>
            ...
            <View style={styles.liveCopy}><Text style={styles.liveLabel}>{t("UPCOMING LIVE CLASS", "आगामी लाइव कक्षा")}</Text><Text numberOfLines={1} style={styles.liveTitle}>{upcomingClass.liveClass.title}</Text><Text style={styles.liveMeta}>{new Date(upcomingClass.liveClass.startsAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</Text></View>
            <MaterialIcons name="chevron-right" size={24} color={COLORS.muted} />
          </Pressable>
        ) : null}
        {user?.role === "student" ? <Pressable accessibilityRole="button" accessibilityLabel={t("Open AI Doubt Solver", "AI डाउट सॉल्वर खोलें")} onPress={() => router.push("/ask-ai")} style={({ pressed }) => [styles.askAi, pressed && styles.pressed]}><IconCircle icon="auto-awesome" size={42} color={COLORS.saffron} background="rgba(255,255,255,0.13)" /><View style={styles.askAiCopy}><Text style={styles.askAiLabel}>{t("STUDY TOOL", "अध्ययन साधन")}</Text><Text style={styles.askAiTitle}>{t("Ask AI · Doubt Solver", "AI से पूछें · डाउट सॉल्वर")}</Text><Text style={styles.askAiBody}>{t("Ask a course doubt, then practise it with a private AI Quiz.", "पाठ्यक्रम का सवाल पूछें और निजी AI क्विज़ से अभ्यास करें।")}</Text></View><MaterialIcons name="arrow-forward" size={23} color={COLORS.white} /></Pressable> : null}
        ...
        <SectionHeading title={t("Study by topic", "विषय के अनुसार पढ़ें")} action={t("Explore", "देखें")} onPress={() => router.push("/explore")} />
        ...
        <SectionHeading title={t("Featured courses", "विशेष पाठ्यक्रम")} action={t("See all", "सभी देखें")} onPress={() => router.push("/explore")} />
...
const styles = StyleSheet.create({
  ...
  notificationButton: { width: 44, height: 44, alignItems: "center", justifyContent: "center", backgroundColor: COLORS.white, borderRadius: 14, borderWidth: 1, borderColor: COLORS.line }, languageButton: { minWidth: 55, height: 44, paddingHorizontal: 8, flexDirection: "row", gap: 3, alignItems: "center", justifyContent: "center", backgroundColor: COLORS.white, borderRadius: 14, borderWidth: 1, borderColor: COLORS.line }, languageCode: { color: COLORS.indigo, fontSize: 10, fontWeight: "900" },
