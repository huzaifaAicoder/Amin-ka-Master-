...
export default function AiQuizScreen() {
  ...
  const exportResultsPdf = async () => {
    ...
    try {
      ...
      const html = buildAiQuizResultsPdfHtml({ topic: topic.trim(), difficulty, questions, answers, score, scorePercent });
      const generated = await Print.printToFileAsync({ html });
      const title = `${topic.trim() || "AI Quiz"} detailed results`;
      const safeTopic = (topic.trim() || "ai-quiz").replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").slice(0, 64);
      const targetUri = `${privateFolder}${Date.now()}-${safeTopic || "ai-quiz"}-results.pdf`;
      await FileSystem.moveAsync({ from: generated.uri, to: targetUri });
      router.push({ pathname: "/pdf-reader", params: { uri: targetUri, title } } as never);
    } catch (error) {
