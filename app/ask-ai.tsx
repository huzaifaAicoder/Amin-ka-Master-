...
export default function AskAiScreen() {
  ...
  return <ScreenContainer className="px-5" edges={["top", "bottom", "left", "right"]}>
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={0} style={styles.keyboardAvoider}>
      ...
      {messages.length === 0 ? <View style={styles.notice}><Text style={styles.noticeText}>Ask about surveying, land measurement, revenue records, or exam preparation. You can attach a map or document image for educational guidance, not official verification.</Text></View> : null}
      ...
      <ScrollView ref={threadRef} style={styles.threadScroll} onContentSizeChange={scrollToLatest} contentContainerStyle={[styles.thread, messages.length === 0 && styles.emptyThread]} showsVerticalScrollIndicator keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === "ios" ? "interactive" : "on-drag"}>
