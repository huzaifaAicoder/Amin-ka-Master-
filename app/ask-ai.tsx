...
export default function AskAiScreen() {
  ...
  const sendQuestion = async () => {
    ...
    try {
      ...
      setMessages((current) => [...current, { id: `assistant-${Date.now()}`, role: "assistant", body: result.answer, placeholder: result.mode === "fallback" }]);
    } catch (error) {
  ...
  return <ScreenContainer className="px-5" edges={["top", "bottom", "left", "right"]}>
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.flex}>
      ...
      <View style={styles.notice}><Tag label="GEMINI READY" tone="green" /><Text style={styles.noticeText}>Ask educational doubts about surveying, land measurement, revenue records, and exam preparation. If the provider is unavailable, the secure fallback keeps the app usable without exposing your question to the client.</Text></View>
