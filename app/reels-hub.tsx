...
import { ActivityIndicator, Alert, FlatList, Linking, Platform, Pressable, RefreshControl, Share, StyleSheet, Text, View } from "react-native";
...
import { formatOfflineSize } from "@/lib/reels-catalogue";

const TABS = ["Liked", "Saved", "Offline"] as const;
type Tab = (typeof TABS)[number];
type Short = { id: number; title: string; description: string | null; videoUrl: string; sourceType: "managed" | "youtube" | "instagram"; likeCount: number; isLiked: boolean; isSaved: boolean; createdAt?: string | Date | null };
...
export default function ReelsHubScreen() {
  ...
  const sortedShorts = useMemo(() => [...shorts].sort((a, b) => sortMode === "title" ? a.title.localeCompare(b.title) : sortMode === "size" ? b.id - a.id : new Date(String(b.createdAt ?? 0)).getTime() - new Date(String(a.createdAt ?? 0)).getTime()), [shorts, sortMode]);
  const sortedOffline = useMemo(() => [...offline].sort((a, b) => sortMode === "title" ? a.title.localeCompare(b.title) : sortMode === "size" ? b.size - a.size : 0), [offline, sortMode]);
  const offlineBytes = useMemo(() => offline.reduce((sum, item) => sum + item.size, 0), [offline]);
  const sortLabel = sortMode === "recent" ? "Recent" : sortMode === "title" ? "Name" : "Size";
  const cycleSort = () => setSortMode((mode) => mode === "recent" ? "title" : mode === "title" ? "size" : "recent");
...
function ShortCard({ item, onToggle, busy }: { item: Short; onToggle: () => void; busy: boolean }) {
  ...
  return <View style={styles.card}>{item.sourceType === "managed" ? <VideoView player={player} nativeControls contentFit="cover" style={styles.video} /> : <ExternalHubMedia item={item} />}<View style={styles.cardBody}><View style={styles.copy}><Text style={styles.cardTitle}>{item.title}</Text>{item.description ? <Text style={styles.cardDescription}>{item.description}</Text> : null}<Text style={styles.cardMeta}>{item.likeCount} {item.likeCount === 1 ? "like" : "likes"}</Text></View><View style={styles.actions}><Pressable accessibilityRole="button" accessibilityLabel={`Share ${item.title}`} onPress={() => void share()} style={styles.iconButton}><MaterialIcons name="share" size={20} color={COLORS.indigo} /></Pressable><Pressable accessibilityRole="button" accessibilityState={{ busy }} onPress={onToggle} disabled={busy} style={styles.removeButton}><MaterialIcons name={item.isLiked ? "favorite" : item.isSaved ? "bookmark-remove" : "bookmark-remove"} size={18} color={COLORS.white} /><Text style={styles.removeText}>{item.isLiked ? "Unlike" : "Remove"}</Text></Pressable></View></View></View>;
}

function ExternalHubMedia({ item }: { item: Short }) {
  const [failed, setFailed] = useState(false);
  const provider = item.sourceType === "youtube" ? "youtube" : "instagram";
  const embedUrl = getExternalMediaEmbedUrl(provider, item.videoUrl);
  const openProvider = async () => { try { await Linking.openURL(item.videoUrl); } catch { Alert.alert("External video unavailable", "This provider link could not be opened on this device."); } };
  if (embedUrl && !failed) return <ExternalMediaPlayer sourceType={provider} uri={embedUrl} onFailure={() => setFailed(true)} style={styles.video} />;
  return <View accessibilityLiveRegion="polite" style={styles.external}><MaterialIcons name={item.sourceType === "youtube" ? "smart-display" : "play-circle-filled"} size={38} color={COLORS.saffron} /><Text style={styles.externalText}>{item.sourceType === "youtube" ? "YouTube playback unavailable" : "Instagram playback unavailable"}</Text><Text style={styles.externalMeta}>This provider did not allow inline playback on this device.</Text><Pressable accessibilityRole="button" onPress={() => void openProvider()} style={styles.externalAction}><MaterialIcons name="open-in-new" size={16} color={COLORS.white} /><Text style={styles.externalActionText}>Open provider</Text></Pressable></View>;
}
...
const styles = StyleSheet.create({ hubTools: { minHeight: 38, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 9 }, sortButton: { minHeight: 36, paddingHorizontal: 11, borderRadius: 11, backgroundColor: COLORS.indigoSoft, flexDirection: "row", alignItems: "center", gap: 5 }, sortText: { color: COLORS.indigo, fontSize: 11, fontWeight: "900" }, storageText: { color: COLORS.muted, fontSize: 11, fontWeight: "700", flexShrink: 1, textAlign: "right" }, header: { paddingTop: 10, paddingBottom: 14, flexDirection: "row", alignItems: "center", gap: 11 }, back: { width: 40, height: 40, borderRadius: 13, alignItems: "center", justifyContent: "center", backgroundColor: COLORS.indigoSoft }, headerCopy: { flex: 1 }, eyebrow: { color: COLORS.earth, fontSize: 10, fontWeight: "900", letterSpacing: 0.8 }, title: { color: COLORS.ink, fontSize: 26, fontWeight: "900" }, subtitle: { color: COLORS.muted, fontSize: 12, marginTop: 2 }, tabs: { flexDirection: "row", gap: 7, marginBottom: 13 }, tab: { flex: 1, minHeight: 42, borderRadius: 13, borderWidth: 1, borderColor: COLORS.line, backgroundColor: COLORS.white, flexDirection: "row", gap: 5, alignItems: "center", justifyContent: "center" }, tabActive: { backgroundColor: COLORS.indigo, borderColor: COLORS.indigo }, tabText: { color: COLORS.indigo, fontSize: 11, fontWeight: "900" }, tabTextActive: { color: COLORS.white }, list: { gap: 13, paddingBottom: 30 }, emptyList: { flexGrow: 1, justifyContent: "center", paddingBottom: 80 }, center: { flex: 1, alignItems: "center", justifyContent: "center", padding: 24, gap: 12 }, muted: { color: COLORS.muted, fontSize: 13 }, retry: { minHeight: 42, paddingHorizontal: 15, borderRadius: 12, backgroundColor: COLORS.indigo, flexDirection: "row", alignItems: "center", gap: 6 }, retryText: { color: COLORS.white, fontWeight: "900" }, card: { overflow: "hidden", borderRadius: 19, borderWidth: 1, borderColor: COLORS.line, backgroundColor: COLORS.white }, video: { width: "100%", aspectRatio: 16 / 9, backgroundColor: COLORS.indigo }, external: { width: "100%", aspectRatio: 16 / 9, alignItems: "center", justifyContent: "center", gap: 6, backgroundColor: "#101A32", paddingHorizontal: 20 }, externalText: { color: COLORS.white, fontSize: 16, fontWeight: "900", textAlign: "center" }, externalMeta: { color: "#D7DFEC", fontSize: 11, textAlign: "center" }, externalAction: { minHeight: 36, paddingHorizontal: 12, borderRadius: 11, marginTop: 3, backgroundColor: COLORS.indigo, flexDirection: "row", alignItems: "center", gap: 5 }, externalActionText: { color: COLORS.white, fontSize: 11, fontWeight: "900" }, cardBody: { padding: 13, gap: 12 }, copy: { flex: 1, gap: 4 }, cardTitle: { color: COLORS.ink, fontSize: 16, fontWeight: "900" }, cardDescription: { color: COLORS.muted, fontSize: 13, lineHeight: 18 }, cardMeta: { color: COLORS.indigo, fontSize: 11, fontWeight: "800" }, actions: { flexDirection: "row", justifyContent: "flex-end", gap: 9 }, iconButton: { width: 42, height: 42, borderRadius: 13, alignItems: "center", justifyContent: "center", backgroundColor: COLORS.indigoSoft }, removeButton: { minHeight: 42, borderRadius: 13, paddingHorizontal: 13, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 6, backgroundColor: COLORS.indigo }, removeText: { color: COLORS.white, fontSize: 12, fontWeight: "900" }, offline: { minHeight: 74, flexDirection: "row", alignItems: "center", gap: 11, padding: 11, borderRadius: 16, backgroundColor: COLORS.white, borderWidth: 1, borderColor: COLORS.line }, offlineIcon: { width: 42, height: 42, borderRadius: 13, alignItems: "center", justifyContent: "center", backgroundColor: "#FDECEA" }, pressed: { opacity: 0.74, transform: [{ scale: 0.985 }] } });
