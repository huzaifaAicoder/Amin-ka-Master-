...
| Student resources had no dedicated offline library, and the AI header consumed unnecessary conversation space. | **GREEN** | Added a Student **Downloads** tab that scans only the app-private `protected-resources/` directory and routes PDFs/videos only into the in-app protected viewers; no share or gallery export action is provided. Condensed the AI header and keeps its message thread flexible. |
| Approved recorded course videos could not be taken offline through the same protected path as PDFs. | **GREEN** | Generalized the Student-only resource authorization helper to accept only published, explicitly download-enabled managed PDF/video resources after active-enrollment verification. Each request receives a signed URL, records its actual `pdf`/`video` audit type, downloads into app-private storage, and opens only in the protected internal PDF/video viewer. Staff must explicitly enable the existing download toggle; external URLs remain ineligible because a managed storage key is required. |

> For a bare Android variant outside Expo-managed configuration, the native activity must apply `WindowManager.LayoutParams.FLAG_SECURE`. In this Expo implementation, the authenticated Student root mounts `preventScreenCaptureAsync`, which requests the Android secure-window behavior across protected Student routes. Real-device confirmation remains necessary because OEM recording overlays and policy behavior can vary.
