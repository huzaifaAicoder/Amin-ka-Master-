...
The **Developer Portal** link appears discretely beneath the Student, Staff/Admin, and Owner cards on the public sign-in screen and opens the private **`/dev-portal`** route. As an additional discovery path, exactly **seven consecutive taps within three seconds** on the Amin Ka Master logo also open that route. The visible low-prominence link remains because it was an established accessibility/discoverability requirement; neither path bypasses authentication. Its first setup and each Developer login require the server-only `DEVELOPER_PORTAL_PASSKEY`; no fallback credential is permitted. Configure this strong passkey through the secure environment settings and retain it outside normal business staff channels. The portal stores display-only branding and theme metadata and presents only a configured/not-configured status for Gemini and Razorpay. It never displays or stores a raw provider credential in the mobile client.

The Developer-only **System health / AI monitor** is a read-only operational diagnostic. It reports sanitized API and database-probe status with safe recommendations and a manual refresh control. It deliberately never returns raw error logs, stack traces, connection strings, provider secrets, CAPTCHA tokens, or OTP values, and it cannot auto-generate or apply source-code changes. A hardcoded Developer OTP/master mobile workflow and a mandatory online CAPTCHA are not implemented: hardcoding authentication material is insecure, while CAPTCHA requires a configured provider and server-side verification. Use a real managed MFA and CAPTCHA provider only after credentials, delivery rules, rate limits, recovery, and monitoring are designed and configured.
